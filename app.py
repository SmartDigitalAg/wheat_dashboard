...
def chart_ndvi(roi1, roi2):
    ...
    c_l9_field2 = draw_chart(l9_field2, "yellow")
...
def load_background():
    ...
    m = folium.Map(
        location=[36.017287819982705, 126.9094862178911],
        zoom_start=17,
        tiles=tiles,
        attr=attr)
    return m
...
def wheat_chart():
    st.subheader("Iksan Wheat Yield Chart")
    df_wheat = pd.read_csv("전라북도_익산시.csv")
    df_wheat = df_wheat[(df_wheat['lo3'] == '함라면') & (df_wheat['item'] == '단위생산량')][['year', 'value']]
    df_wheat["year_date"] = pd.to_datetime(dict(year=df_wheat["year"], month=1, day=1))
    df_wheat["year"] = df_wheat["year"].astype(int)

    line_chart = alt.Chart(df_wheat).mark_bar().encode(
        y=alt.Y('value', title='Yield(kg/a)'),
        x=alt.X('year_date', title='Year', axis = alt.Axis(title = 'Date'.upper(), format = ("%Y")))
    )
